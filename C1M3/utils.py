...
def generate_embedding(prompt: str, model: str = "BAAI/bge-base-en-v1.5", together_api_key = None, **kwargs):
    ...
    if (not together_api_key) and ('TOGETHER_API_KEY' not in os.environ):
        client = OpenAI(
    api_key = os.getenv("TOGETHER_API_KEY", ""), # Set any as dlai proxy does not use it. Set the together api key if using the together endpoint
    base_url="https://api.together.xyz", # If using together endpoint, add it here https://api.together.xyz/
